...
public:
	ServerConfigs();
	// Loads the internal hashmap with the configs file's content.
	void LoadHeadersFromConfigFile();
	// Returns a const reference to the hashmap
	const std::unordered_map<std::string, std::string>& GetConfigs() const;
private:
	// This object's internal hashmap.
	...
	// Returns the object's internal hashmap for reading only.
